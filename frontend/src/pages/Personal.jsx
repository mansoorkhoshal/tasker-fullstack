  ...
  useEffect(() => {
    const fetchPersonalTasks = async () => {
      try {
        ...
        const onlyPersonal = Array.isArray(json.data)
          ? json.data.filter(
              (task) => task.categoryId?.Name?.toLowerCase() === 'personal'
            )
          : [];

        setPersonalTasks(onlyPersonal);
      } catch (error) {
  ...
  return (
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
      <div className='flex justify-between items-center gap-4 my-6 bg-white/80 backdrop-blur-md border border-blue-100 rounded-2xl p-5 shadow-md bg-linear-to-br from-blue-50 via-white to-blue-100'>
        <h1 className='text-2xl sm:text-3xl font-bold text-gray-700'>Personal Tasks</h1>
      </div>
